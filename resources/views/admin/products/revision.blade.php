    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Persetujuan Revisi Produk</h1>
            </div>
        </div>

        <div class="bg-white dark:bg-gray-800 shadow-xs rounded-xl">
            <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
                <h2 class="font-semibold text-gray-800 dark:text-gray-100">Daftar Revisi Pending</h2>
            </header>
            <div class="p-3">
                    ...
                    <table class="table-auto w-full">
                        <thead class="text-xs font-semibold uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                <th class="p-2 text-left">Keterangan Revisi</th>
                        ...
                        <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                            @forelse ($revisions as $index => $revision)
                                <tr>
                                    <td class="p-2 text-center">{{ $revisions->firstItem() + $index }}</td>
                                    <td class="p-2">
                                        {{ $revision->product->name ?? '-' }}
                                    </td>
                                    <td class="p-2">
                                        {{ $revision->product->merchant->name ?? '-' }}
                                    </td>
                                    <td class="p-2">
                                        {{ $revision->notes ?? '-' }}
                                    </td>
                                    ...
                                    <td class="p-2 text-center">
                                        <a href="{{ route('admin.product-revisions.show', $revision) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="p-2 text-center text-gray-500">Belum ada revisi pending.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
