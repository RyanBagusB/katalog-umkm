                ...
                <ul class="mt-3 space-y-1">
                    @php
                        $menuItems = [
                            [
                                'label' => 'Dashboard',
                                'icon' => 'M10.707 1.293a1 1 0 00-1.414 0L2 8.586V13a1 1 0 001 1h3v-3a1 1 0 011-1h2a1 1 0 011 1v3h3a1 1 0 001-1V8.586l-7.293-7.293z',
                                'slug' => 'dashboard',
                                'routes' => ['dashboard'],
                                'children' => [
                    ...
                    @foreach ($menuItems as $item)
                        <li class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 bg-linear-to-r @if(in_array(Request::segment(1), [$item['slug']])) from-violet-500/[0.12] dark:from-violet-500/[0.24] to-violet-500/[0.04] @endif"
                            x-data="{ open: {{ in_array(Request::segment(1), [$item['slug']]) ? 1 : 0 }} }">
                            <a href="#"
                                class="block text-gray-800 dark:text-gray-100 truncate transition @if(!in_array(Request::segment(1), [$item['slug']])) hover:text-gray-900 dark:hover:text-white @endif"
                                @click.prevent="open = !open; sidebarExpanded = true">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        <svg class="shrink-0 fill-current @if(in_array(Request::segment(1), [$item['slug']])) text-violet-500 @else text-gray-400 dark:text-gray-500 @endif"
                                ...
                                <ul class="pl-8 mt-1" :class="open ? 'block' : 'hidden'">
                                    @foreach ($item['children'] as $child)
                                        <li class="mb-1 last:mb-0">
                                            <a class="block text-gray-500/90 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition truncate @if(Route::is($child['route'])) text-violet-500! @endif"
